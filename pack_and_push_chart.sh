...
HELM_CHARTS_FOLDERS=$3
CHART_VERSION=$4  # Optional: override version from Chart.yaml
...
helm_pack () {
  for i in "${@}"; do
    ...
    if [[ -n "$CHART_VERSION" ]]; then
      echo "Using version override: $CHART_VERSION"
      helm package "$i" --version "$CHART_VERSION"
    else
      echo "Using version from Chart.yaml"
      helm package "$i"
    fi
  done
}
