...
from mfa import disable_mfa, enable_mfa, get_public_mfa_state, provision_totp, validate_totp_secret
...
def mail_user_privs_remove():
    return add_remove_mail_user_privilege(
        request.form.get("email", ""), request.form.get("privilege", ""), "remove", env
    )
...
def ssl_get_status():
    ...
    from web_update import get_web_domains, get_web_domains_info
    ...
    domains_status = [
        {
            ...
            "text": d["ssl_certificate"][1]
            + (" " + cant_provision[d["domain"]] if d["domain"] in cant_provision else ""),
        }
...
def web_update():
    ...
    print("DEBUG: /web/update API endpoint called", file=sys.stderr, flush=True)
    from web_update import do_web_update

    result = do_web_update(env)
    print(f"DEBUG: /web/update API endpoint returning: {result}", file=sys.stderr, flush=True)
    return result
...
def munin_start():
    ...
    response.set_cookie(
        "session",
        auth_service.create_session_key(request.user_email, env, type="cookie"),
        max_age=60 * 30,
        secure=True,
        httponly=True,
        samesite="Strict",
