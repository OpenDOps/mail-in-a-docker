...
def load_environment():
    ...
    file_env = load_env_vars_from_file("/etc/mailinabox.conf")

    # Start with os.environ (runtime values from docker-compose) as base
    # This gives priority to docker-compose environment variables
    env = collections.OrderedDict()
    for key, value in os.environ.items():
        env[key] = value

    # Merge file values as fallback for keys not in os.environ
    # This ensures we have all keys from both sources
...
def create_syslog_handler():
    # Create a logging handler appropriate for the environment.
    # In Docker containers, use stderr (which is captured by docker logs).
    # Otherwise, try to use syslog.
    import logging
    import logging.handlers
    import sys

    # Check if we're in Docker (check for IN_A_DOCKER env var or if /dev/log doesn't exist)
    in_docker = os.environ.get("IN_A_DOCKER", "false").lower() == "true"
    has_syslog = os.path.exists("/dev/log")

    if in_docker or not has_syslog:
        # In Docker: use StreamHandler to stderr (captured by docker logs)
        handler = logging.StreamHandler(sys.stderr)
        handler.setLevel(logging.WARNING)
        # Format messages similar to syslog format for consistency
        formatter = logging.Formatter("%(name)s[%(process)d]: %(levelname)s: %(message)s")
        handler.setFormatter(formatter)
        return handler

    # Not in Docker and syslog is available: use SysLogHandler
    try:
