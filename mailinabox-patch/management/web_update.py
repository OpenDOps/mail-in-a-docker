...
from utils import safe_domain_name, shell, sort_domains
...
def get_domains_with_a_records(env):
    ...
    for domain, rtype, value in dns:
        if rtype == "CNAME" or (
            rtype in {"A", "AAAA"} and value not in {"local", env["PUBLIC_IP"], env["PUBLIC_IPV6"]}
        ):
            domains.add(domain)
    return domains
...
def get_web_domains_with_root_overrides(env):
    ...
    if os.path.exists(nginx_conf_custom_fn):
        ...
        for domain, settings in custom_settings.items():
            for type, value in [
                ("redirect", settings.get("redirects", {}).get("/")),
...
def do_web_update(env):
    ...
    nginx_conf = re.sub(
        r"unix:/var/run/php/php\d+\.\d+-fpm\.sock", f"unix:/var/run/php/php{php_ver_dir}-fpm.sock", nginx_conf
    )
    ...
    if env.get("IN_KUBERNETES", "false") == "true":
        ...
        print("DEBUG: Kubernetes mode detected - skipping nginx config generation", file=sys.stderr, flush=True)
        print("DEBUG: Ingress resources should be managed via Kubernetes API", file=sys.stderr, flush=True)
    ...
    if use_docker_volume:
        ...
        print("DEBUG: Reloading nginx service directly", file=sys.stderr, flush=True)
...
def make_domain_config(domain, templates, ssl_certificates, env):
    ...
    return nginx_conf.replace(
        "$REDIRECT_DOMAIN", re.sub(r"^www\.", "", domain)
