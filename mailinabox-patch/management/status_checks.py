...
import psutil
from backup import backup_status, get_backup_config
from dns_update import (
    build_tlsa_record,
...
from mailconfig import get_mail_aliases, get_mail_domains
from ssl_certificates import check_certificate, get_domain_ssl_files, get_ssl_certificates
from utils import get_ssh_config_value, get_ssh_port, load_env_vars_from_file, load_settings, shell, sort_domains
from web_update import get_domains_with_a_records, get_web_domains
    ...
    if ufw[0] == "Status: active":
        ...
        for service in get_services():
            if service["public"] and not is_port_allowed(ufw, service["port"]):
                not_allowed_ports += 1
                output.print_error(
                    "Port {} ({}) should be allowed in the firewall, please re-run the setup.".format(
...
def check_free_disk_space(rounded_values, env, output):
    ...
    elif bytes_free > 0.15 * bytes_total:
        if rounded_values:
    ...
    if backup_cache_count > 1:
        output.print_warning(
            f"The backup cache directory {backup_cache_path} has more than one backup target cache. Consider clearing this directory to save disk space."
        )
...
def check_backup(rounded_values, env, output):
    ...
    if backups and len(backups) > 0:
        ...
        output.print_error(
            "Could not obtain backup status or no backup has been made (yet). "
...
def evaluate_spamhaus_lookup(lookupaddress, lookuptype, lookupdomain, output, zen):
    ...
    if zen is None:
        ...
        output.print_warning(
            f"""Connection to zen.spamhaus.org timed out. Could not determine whether this box's
            {lookuptype} address is blacklisted. Please try again later."""
        )
    elif zen == "[Not Set]":
        output.print_warning(
            f"""Could not connect to zen.spamhaus.org. Could not determine whether this box's
			{lookuptype} address is blacklisted. Please try again later."""
        )
    elif zen == "127.255.255.252":
        output.print_warning(
            f"""Incorrect spamhaus query: {lookupdomain + '.zen.spamhaus.org'}. Could not determine whether
            this box's {lookuptype} address is blacklisted."""
        )
    elif zen == "127.255.255.254":
        output.print_warning(
            f"""Mail-in-a-Box is configured to use a public DNS server. This is not supported by
			spamhaus. Could not determine whether this box's {lookuptype} address is blacklisted."""
        )
    elif zen == "127.255.255.255":
        output.print_warning(
            f"""Too many queries have been performed on the spamhaus server. Could not determine
            whether this box's {lookuptype} address is blacklisted."""
        )
    else:
        output.print_error(
            f"""The {lookuptype} address of this machine {lookupaddress} is listed in the Spamhaus Block
...
def run_domain_checks(rounded_time, env, output, pool, domains_to_check=None):
    ...
    args = (
        (domain, rounded_time, env, dns_domains, dns_zonefiles, mail_domains, web_domains, domains_with_a_records)
...
def check_primary_hostname_dns(domain, env, output, dns_domains, dns_zonefiles):
    ...
    if ns_ips == env["PUBLIC_IP"] + "/" + env["PUBLIC_IP"]:
        output.print_ok(
            "Nameserver glue records are correct at registrar. [ns1/ns2.{} ↦ {}]".format(
        ...
        output.print_warning(
            """Nameserver glue records (ns1.{} and ns2.{}) should be configured at your domain name
			registrar as having the IP address of this box ({}). They currently report addresses of {}. If you have set up External DNS, this may be OK.""".format(
        ...
        output.print_error(
            """Nameserver glue records are incorrect. The ns1.{} and ns2.{} nameservers must be configured at your domain name
			registrar as having the IP address {}. They currently report addresses of {}. It may take several hours for
			public DNS to update after a change.""".format(
                env["PRIMARY_HOSTNAME"], env["PRIMARY_HOSTNAME"], env["PUBLIC_IP"], ns_ips
            )
        )
    ...
    if ip == env["PUBLIC_IP"] and not (ipv6 and env["PUBLIC_IPV6"] and ipv6 != normalize_ip(env["PUBLIC_IPV6"])):
        ...
        output.print_error(
            """This domain must resolve to this box's IP address ({}) in public DNS but it currently resolves
    ...
    if existing_rdns_v4 == domain and existing_rdns_v6 in {None, domain}:
        ...
        output.print_error(
            f"""This box's reverse DNS is currently {existing_rdns_v4}, but it should be {domain}. Your ISP or cloud provider will have instructions
			on setting up reverse DNS for this box."""
        )
    else:
        output.print_error(
            f"""This box's reverse DNS is currently {existing_rdns_v4} (IPv4) and {existing_rdns_v6} (IPv6), but it should be {domain}. Your ISP or cloud provider will have instructions
			on setting up reverse DNS for this box."""
        )
    ...
    if tlsa25 == tlsa25_expected:
        output.print_ok(
            f"""The DANE TLSA record for incoming mail is correct ({tlsa_qname}).""",
        )
    elif tlsa25 is None:
        if has_dnssec:
        ...
        output.print_error(
            f"""The DANE TLSA record for incoming mail ({tlsa_qname}) is not correct. It is '{tlsa25}' but it should be '{tlsa25_expected}'.
...
def check_alias_exists(alias_name, alias, env, output):
    ...
    if alias in mail_aliases:
        if mail_aliases[alias]:
            ...
            output.print_error(
                f"""You must set the destination of the mail alias for {alias} to direct email to you or another administrator."""
            )
    else:
        output.print_error(
            f"""You must add a mail alias for {alias} which directs email to you or another administrator."""
        )
...
def check_dns_zone(domain, env, output, dns_zonefiles):
    ...
    if existing_ns.lower() == correct_ns.lower():
        ...
        output.print_warning(
            f"""The nameservers set on this domain at your domain name registrar should be {correct_ns}. They are currently {existing_ns}.
			If you are using External DNS, this may be OK."""
        )
        probably_external_dns = True
    else:
        output.print_error(
            f"""The nameservers set on this domain are incorrect. They are currently {existing_ns}. Use your domain name registrar's
    ...
    if custom_secondary_ns and not probably_external_dns:
        ...
        for ns in custom_secondary_ns:
            ...
            if ip == correct_ip:
                ...
                output.print_error(
                    f"Secondary nameserver {ns} is not configured correctly. (It resolved this domain as {ip}. It should be {correct_ip}.)"
                )
            ...
            if checkSOA:
                ...
                if SOARecord == SOASecondary:
                    ...
                    output.print_error(
                        f"""Secondary nameserver {ns} has inconsistent SOA record (primary: {SOARecord} versus secondary: {SOASecondary}).
...
def check_dns_zone_suggestions(domain, env, output, dns_zonefiles, domains_with_a_records):
    ...
    if "www." + domain in domains_with_a_records:
        output.print_warning(
            f"""A redirect from 'www.{domain}' has been disabled for this domain because you have set a custom DNS record on the www subdomain."""
        )
...
def check_dnssec(domain, env, output, dns_zonefiles, is_checking_primary=False):
    ...
    with open(ds_file, encoding="utf-8") as f:
        for rr_ds in f:
            ...
            dnssec_keys = load_env_vars_from_file(
                os.path.join(env["STORAGE_ROOT"], f"dns/dnssec/{alg_name_map[ds_alg]}.conf")
            )
            with open(
                os.path.join(env["STORAGE_ROOT"], "dns/dnssec/" + dnssec_keys["KSK"] + ".key"), encoding="utf-8"
            ) as f:
    ...
    if len(ds) == 0:
        ...
        if matched_ds:
            ...
            if {r[1] for r in matched_ds} == {"13"} and {r[2] for r in matched_ds} <= {
                "2",
                "4",
...
def check_mail_domain(domain, env, output):
    ...
    if mxhost is None:
        ...
        # probably confuse DANE TLSA, but we'll let that slide for now.
            ...
            if domain_a is not None and domain_a == primary_a:
                output.print_ok(
                    f"Domain's email is directed to this domain. [{domain} has no MX record but its A record is OK]"
                )
            else:
                output.print_error(
                    f"""This domain's DNS MX record is not set. It should be '{recommended_mx}'. Mail will not
        ...
        if valid == postfix_mta_sts_resolver.resolver.STSFetchResult.VALID:
            if (
                policy[1].get("mx") == [env["PRIMARY_HOSTNAME"]] and policy[1].get("mode") == "enforce"
        ...
        output.print_error(
            f"""This domain's DNS MX record is incorrect. It is currently set to '{mx}' but should be '{recommended_mx}'. Mail will not
    ...
    if dbl is None:
        ...
        output.print_warning(
            f"Connection to dbl.spamhaus.org timed out. Could not determine whether the domain {domain} is blacklisted. Please try again later."
        )
    elif dbl == "[Not Set]":
        output.print_warning(
            f"Could not connect to dbl.spamhaus.org. Could not determine whether the domain {domain} is blacklisted. Please try again later."
        )
    elif dbl == "127.255.255.252":
        output.print_warning(
            "Incorrect spamhaus query: {}. Could not determine whether the domain {} is blacklisted.".format(
                domain + ".dbl.spamhaus.org", domain
            )
        )
    elif dbl == "127.255.255.254":
        output.print_warning(
            f"Mail-in-a-Box is configured to use a public DNS server. This is not supported by spamhaus. Could not determine whether the domain {domain} is blacklisted."
        )
    elif dbl == "127.255.255.255":
        output.print_warning(
            f"Too many queries have been performed on the spamhaus server. Could not determine whether the domain {domain} is blacklisted."
        )
    else:
        output.print_error(
            f"""This domain is listed in the Spamhaus Domain Block List (code {dbl}),
    ...
    if domain != env["PRIMARY_HOSTNAME"]:
        ...
        for rtype, expected in (("A", env["PUBLIC_IP"]), ("AAAA", env.get("PUBLIC_IPV6"))):
            ...
            if value == normalize_ip(expected):
                ...
                output.print_error(
                    f"""This domain should resolve to this box's IP address ({rtype} {expected}) if you would like the box to serve
...
def what_version_is_this(env):
    ...
    return shell(
        "check_output",
        ["/usr/bin/git", "describe", "--always", "--abbrev=0"],
        env={"GIT_DIR": os.path.join(miab_dir, ".git")},
    ).strip()
...
def get_latest_miab_version():
    ...
    try:
        return (
            re.search(b"TAG=(.*)", urlopen("https://mailinabox.email/setup.sh?ping=1", timeout=5).read())
            .group(1)
            .decode("utf8")
        )
...
def check_miab_version(env, output):
    ...
    if IN_KUBERNETES:
        ...
        if this_ver == latest_ver:
            ...
            output.print_error(
                f"Latest Mail-in-a-Box version could not be determined. You are running version {this_ver}."
            )
        else:
            output.print_error(
                f"A new version of Mail-in-a-Box is available. You are running version {this_ver}. The latest version is {latest_ver}. For upgrade instructions, see https://mailinabox.email. "
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        if not os.path.exists(tls_cert["certificate"]):
            sys.exit(1)
        cert_status, cert_status_details = check_certificate(
            domain, tls_cert["certificate"], tls_cert["private-key"], warn_if_expiring_soon=False
        )
        if cert_status != "OK":
            sys.exit(1)
        sys.exit(0)
