...
# Trace database state during build
echo "Debug users"
echo "DEBUG: setup_mail_users.sh - STORAGE_ROOT=$STORAGE_ROOT"
echo "DEBUG: setup_mail_users.sh - db_path=$db_path"
echo "DEBUG: setup_mail_users.sh - db_path exists: $([ -f "$db_path" ] && echo "YES" || echo "NO")"
...
if [ ! -f "$db_path" ]; then
  ...
  # System user will be added by get_user_secret.py daemon task
	echo "CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT NOT NULL UNIQUE, password TEXT NOT NULL, extra, privileges TEXT NOT NULL DEFAULT '', quota TEXT NOT NULL DEFAULT '0', is_system BOOLEAN NOT NULL DEFAULT FALSE);" | sqlite3 "$db_path";
	echo "CREATE TABLE aliases (id INTEGER PRIMARY KEY AUTOINCREMENT, source TEXT NOT NULL UNIQUE, destination TEXT NOT NULL, permitted_senders TEXT);" | sqlite3 "$db_path";
	echo "CREATE TABLE mfa (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, type TEXT NOT NULL, secret TEXT NOT NULL, mru_token TEXT, label TEXT, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE);" | sqlite3 "$db_path";
	echo "CREATE TABLE auto_aliases (id INTEGER PRIMARY KEY AUTOINCREMENT, source TEXT NOT NULL UNIQUE, destination TEXT NOT NULL, permitted_senders TEXT);" | sqlite3 "$db_path";
	echo "DEBUG: setup_mail_users.sh - Database created"
else
	echo "DEBUG: setup_mail_users.sh - Database already exists"
fi

# Trace tables and users after creation/check
if [ -f "$db_path" ]; then
	echo "DEBUG: setup_mail_users.sh - Database file exists, checking tables..."
	sqlite3 "$db_path" ".tables" | while read -r tables; do
		echo "DEBUG: setup_mail_users.sh - Tables found: $tables"
	done
	echo "DEBUG: setup_mail_users.sh - Checking users table..."
	user_count=$(sqlite3 "$db_path" "SELECT COUNT(*) FROM users" 2>/dev/null || echo "ERROR")
	echo "DEBUG: setup_mail_users.sh - Users count: $user_count"
	echo "DEBUG: setup_mail_users.sh - Database location: $(readlink -f "$db_path" 2>/dev/null || echo "$db_path")"
	echo "DEBUG: setup_mail_users.sh - Database size: $(stat -c%s "$db_path" 2>/dev/null || stat -f%z "$db_path" 2>/dev/null || echo "unknown") bytes"
else
	echo "DEBUG: setup_mail_users.sh - ERROR: Database file does not exist after creation attempt!"
fi
...
# Expand variables in the SQL config file (Dovecot doesn't expand $db_path automatically)
...
# Verify the SQL config file has the correct path
echo "DEBUG: Dovecot SQL config connect path: $(grep '^connect =' /etc/dovecot/dovecot-sql.conf.ext)"
echo "DEBUG: Database file exists: $([ -f "$db_path" ] && echo "YES" || echo "NO")"
echo "DEBUG: Database file readable: $([ -r "$db_path" ] && echo "YES" || echo "NO")"
